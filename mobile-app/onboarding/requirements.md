...
### Onboarding — Parceiros (role = partner)
...
- Criação automática do registo `partner_profile` (estado `draft`, não visível no Marketplace)
...
| RN04 | Parceiros: o `partner_profile` é criado em `draft` assim que nome + categoria estão definidos — reduz custo de abandono a meio, ao contrário de RN03. |
...
RN08 significa que podemos ter NIFs mal preenchidos até ao momento de pagamento — trade-off consciente para reduzir fricção no onboarding, resolvido mais tarde no funil quando o parceiro já está mais investido.
