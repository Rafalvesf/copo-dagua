...
### Específicas
- **Seleção de papel (role)** no registo: Noivo(a) ou Fornecedor. Administradores não se registam publicamente — são criados manualmente via painel interno (`admin-web/users/`).
...
| # | Regra |
|---|---|
| RN01 | Um utilizador tem exatamente um `role` principal: `couple`, `supplier` ou `admin`. Não existem contas híbridas no MVP (uma pessoa que é noiva e também fornecedora precisa de duas contas com emails diferentes). |
...
| RN05 | Contas de Fornecedor só ficam com o perfil "visível" no Marketplace depois de completarem o onboarding de fornecedor (dados fiscais, portefólio mínimo) — validado em `backend/suppliers/`, mas a conta de autenticação já existe antes disso. |
...
| RN08 | Eliminação de conta é "soft delete" durante 30 dias (permite recuperação e cumpre obrigações fiscais/contratuais em curso — ex: um fornecedor não pode desaparecer com um contrato ativo). Eliminação definitiva e anonimização após esse período. |
